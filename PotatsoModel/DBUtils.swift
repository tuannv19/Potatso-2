...
open class DBUtils {
    ...
    open static func markAll(syncd: Bool) throws {
        ...
        for ruleset in mRealm.objects(MRuleSet.self) {
...
extension DBUtils {
    ...
    public static func allObjectsToSyncModified() -> [BaseModel] {
        ...
        let rulesets = mRealm.objects(MRuleSet.self).filter(filter).map({ $0 })
        ...
        var iterator2: LazyMapIterator<RLMIterator<MRuleSet>, MRuleSet>? = nil
...
extension ConfigurationGroup {
    ...
    public static func appendRuleSet(forGroupId groupId: String, rulesetId: String) throws {
        try DBUtils.modify(ConfigurationGroup.self, id: groupId) { (realm, group) -> Error? in
            if let ruleset = DBUtils.get(rulesetId, type: MRuleSet.self, inRealm: realm) {
