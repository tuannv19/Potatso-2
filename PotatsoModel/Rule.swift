...
extension MRuleType {
    ...
    public static func fromInt(_ intValue: Int) -> MRuleType? {
        switch intValue {
        case 1:
            return .Domain
        case 2:
...
extension MRuleType: CustomStringConvertible {
...
public final class Rule {
    ...
    public convenience init(str: String) throws {
        ...
        let value = parts[1]
        guard let type = MRuleType(rawValue: typeStr), let action = RuleAction(rawValue: actionStr), value.characters.count > 0 else {
            throw RuleError.invalidRule(str)
        }
        self.init(type: type, action: action, value: value)
    }
    
    public init(type: MRuleType, action: RuleAction, value: String) {
        self.type = type
        self.value = value
        self.action = action
    }
    ...
    public convenience init?(json: [String: AnyObject]) {
        guard let typeRaw = json["type"] as? String, let type = MRuleType(rawValue: typeRaw) else {
            return nil
        }
