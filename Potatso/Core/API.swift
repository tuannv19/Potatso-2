...
//    static let URL = "http://192.168.2.217:8000/api/"
    ...
    static func getRuleSets(_ page: Int = 1, count: Int = 20, callback: @escaping (Alamofire.DataResponse<[MRuleSet]>) -> Void) {
    ...
    static func getRuleSetDetail(_ uuid: String, callback: @escaping (Alamofire.DataResponse<MRuleSet>) -> Void) {
    ...
    static func updateRuleSetListDetail(_ uuids: [String], callback: @escaping  (Alamofire.DataResponse<[MRuleSet]>) -> Void) {
...
extension MRuleSet {
    ...
    static func addRemoteObject(_ ruleset: MRuleSet, update: Bool = true) throws {
        ruleset.isSubscribe = true
        ruleset.deleted = false
        ruleset.editable = false
        let id = ruleset.uuid
        guard let local = DBUtils.get(id, type: MRuleSet.self) else {
    ...
    static func addRemoteArray(_ rulesets: [MRuleSet], update: Bool = true) throws {
...
extension Rule: Mappable {
    ...
    public convenience init?(map: Map) {
        ...
        guard let typeStr = map.JSON["type"] as? String, let type = MRuleType(rawValue: typeStr) else {
            return nil
        }
